...
const Customer = () => {
  ...
  const watchTransaction = async (CheckoutRequestID: string, clientID: string) => {
    try {
        const response = await fetch(`${config.microserviceUrl}/microservice/watchTransaction`, {
            method: 'POST',
            headers: {
  ...
  const initiatePayment = async () => {
    ...
    setReqLoading(true);
    try {
      const response = await fetch(`${config.microserviceUrl}/microservice/api/payment`, {

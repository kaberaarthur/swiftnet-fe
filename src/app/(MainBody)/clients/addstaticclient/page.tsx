  ...
  useEffect(() => {
    if (formData.router_id && user.company_id) {
      ...
      const fetchHotspotPlans = async () => {
        try {
          const plansResponse = await fetch(`/backend/pppoe-plans?router_id=${formData.router_id}&company_id=${user.company_id}&type=static`);
    ...
    <>
      ...
      <Container fluid>
        <Alert color="success" className="py-2" isOpen={visible} toggle={() => setVisible(false)} fade>
          <p>
            <strong>Static Client Added Successfully!</strong>
          </p>
        </Alert>
        <Row className="g-3">
          <Col sm="6">
